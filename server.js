...
app.use(
  session({
    secret: process.env.SESSION_SECRET || generateStrongSecret(),
    resave: false,
...
// rate limiter pour les opérations POST (publication, modification, suppression)
var postLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 30, // max 30 requêtes par minute par IP
  message: "Trop de requêtes. Veuillez attendre avant de continuer.",
  standardHeaders: false,
  skip: function (req) {
    // ne pas compter les requêtes non-POST
    return req.method !== "POST";
  },
});

// rate limiter plus strict pour la suppression
var deleteLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: 10, // max 10 suppression par minute
  message: "Trop de suppressions. Veuillez attendre avant de continuer.",
});

// générer une clé de session forte si elle n'existe pas
function generateStrongSecret() {
  var strong = crypto.randomBytes(32).toString("hex");
  console.warn(
    "⚠️  SESSION_SECRET générée aléatoirement (non persistée). Définir SESSION_SECRET dans .env pour la production.",
  );
  return strong;
}
...
app.post("/ajouter", postLimiter, requireLogin, async function (req, res) {
...
app.post("/effacer", deleteLimiter, requireLogin, async function (req, res) {
  var user = req.session.user;
...
app.post("/modifier", postLimiter, requireLogin, async function (req, res) {
  var user = req.session.user;
...
app.post("/deplacer", postLimiter, requireLogin, async function (req, res) {
  var user = req.session.user;
  var id = parseInt(req.body.id);
