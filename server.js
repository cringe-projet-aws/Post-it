...
var app = express();
// Utilitaire de chemins cross-platform.
const path = require("path");
// Modules natifs Node pour créer les serveurs HTTPS/HTTP.
const https = require("https");
const http = require("http");
// Lecture des certificats depuis le disque.
const fs = require("fs");
// Hachage/vérification des mots de passe.
const bcrypt = require("bcryptjs");
// Gestion des sessions utilisateur côté serveur.
var session = require("express-session");
// Middleware d'entêtes de sécurité HTTP.
const helmet = require("helmet");
// Protection contre les requêtes CSRF.
var csrf = require("csurf");
// Messages temporaires entre redirections.
var flash = require("connect-flash");
// Limiteur de tentatives sur les routes sensibles.
const rateLimit = require("express-rate-limit");
// Accès à la base de données via Knex.
const db = require("./db");

// Port réseau: variable d'environnement prioritaire, sinon 3000.
...
app.use(
  session({
    // Clé de signature du cookie de session (à mettre dans .env en production).
    secret: process.env.SESSION_SECRET || "notree_cle_par_défaut",
    // Évite de réécrire la session si rien n'a changé.
    resave: false,
    // N'écrit pas de session vide (avant authentification).
    saveUninitialized: false,
    cookie: {
      // Cookie inaccessible au JavaScript côté navigateur.
      httpOnly: true,
      // Cookie envoyé uniquement en HTTPS si HTTPS actif.
      secure: certOptions !== null,
      // Protection CSRF supplémentaire côté navigateur.
      sameSite: "strict",
...
app.use(function (req, res, next) {
  // Utilisateur connecté injecté pour l'affichage conditionnel dans les vues.
  res.locals.user = req.session.user || null;
  // Token CSRF injecté pour tous les formulaires.
  res.locals.csrfToken = req.csrfToken();
  // Messages flash d'erreur/succès accessibles depuis les templates.
...
function requireLogin(req, res, next) {
  // Si pas de session utilisateur, refuse l'accès.
  if (!req.session.user) {
    // Détecte un appel AJAX JSON pour répondre en JSON plutôt qu'en redirection.
    var isAjax =
...
async function initialiserBase() {
  ...
  if (!admin) {
    ...
    if (!process.env.ADMIN_PASSWORD) {
      console.warn(
        "⚠️  ADMIN_PASSWORD non défini dans .env - mot de passe par défaut utilisé",
      );
    }
  }
}
...
app.post("/signup", async function (req, res) {
  // Normalise les champs reçus du formulaire.
  ...
  try {
    ...
    // Hash sécurisé du mot de passe avant stockage en base.
  ...
  try {
    ...
    req.session.regenerate(function (err) {
      ...
      req.session.user = {
        ...
        // Conversion explicite 0/1 -> booléens pour simplifier les contrôles côté code.
...
app.post("/ajouter", requireLogin, async function (req, res) {
  // Récupère les permissions de l'utilisateur courant depuis la session.
  ...
  if (!user.can_create) {
    return res
      ...
      .json({
        ok: false,
        message: "Vous n'avez pas la permission de créer des postits",
      });
  }

  var texte = (req.body.texte || "").trim();
  // Position initiale du post-it convertie en entiers.
  ...
  if (texte.length > 500) {
    return res
      .status(400)
      .json({ ok: false, message: "Texte trop long (500 caractères max)" });
  }
  if (x < 0) x = 0;
  if (y < 0) y = 0;
  ...
  try {
    ...
    var res2 = await db("postits")
      .where("tableau_id", board)
      .max("z_index as max")
      .first();
...
app.post("/effacer", requireLogin, async function (req, res) {
  ...
  if (!user.can_delete) {
    return res
      .status(403)
      .json({ ok: false, message: "Pas la permission de supprimer" });
  }
  ...
  try {
    ...
    if (postit.auteur_id != user.id && !user.can_admin) {
      return res
        .status(403)
        .json({ ok: false, message: "Ce postit ne vous appartient pas" });
    }
...
app.post("/modifier", requireLogin, async function (req, res) {
  ...
  if (!user.can_edit) {
    return res
      .status(403)
      .json({ ok: false, message: "Pas la permission de modifier" });
  }

  // Identifiant du post-it concerné.
  var id = parseInt(req.body.id);
  var texte = (req.body.texte || "").trim();
...
app.post("/deplacer", requireLogin, async function (req, res) {
  // Données de déplacement envoyées par le front (drag and drop).
  var user = req.session.user;
  var id = parseInt(req.body.id);
...
app.get("/admin", requireLogin, async function (req, res) {
  // Contrôle d'accès strict: seuls les admins voient cette page.
...
app.post("/admin/update", requireLogin, async function (req, res) {
  // Vérifie le rôle admin avant modification des permissions.
  if (!req.session.user.can_admin) {
    return res.status(403).send("Accès refusé");
  }

  // Utilisateur cible à mettre à jour.
  ...
  try {
    ...
    // Indique si l'admin modifie son propre compte.
    var estSoi = userId == req.session.user.id;
    // Les cases à cocher HTML sont converties en 1/0 pour la DB.
...
app.post("/admin/delete", requireLogin, async function (req, res) {
  ...
  try {
    ...
    req.flash(
      "succes",
      "Utilisateur " + target.username + " supprimé avec succès",
    );
    res.redirect("/admin");
  } catch (err) {
...
app.get("/:board", async function (req, res) {
  // Récupère le nom du tableau depuis l'URL dynamique.
  var board = (req.params.board || "default").toLowerCase();
  if (!slugValide(board)) {
    return res
      .status(400)
      .send(
        "Nom de tableau invalide (lettres minuscules, chiffres, - et _ uniquement)",
      );
  }
  try {
    var postits = await db("postits")
    ...
    // Rendu serveur de la page principale avec post-its déjà chargés.
...
app.use(function (err, req, res, next) {
  // Cas géré explicitement: token CSRF absent/incorrect.
  if (err.code === "EBADCSRFTOKEN") {
    ...
    if (isAjax) {
      return res
        .status(403)
        .json({ ok: false, message: "Token invalide, rechargez la page" });
    }
    return res.status(403).send("Requête invalide - rechargez la page");
  }
  // Journalise les erreurs inattendues côté serveur.
...
if (certOptions) {
  // Mode production/local sécurisé: HTTPS activé si certificats présents.
  initialiserBase()
    .then(function () {
      // lancer en HTTPS
      ...
      http
        .createServer(function (req, res) {
          // Construit l'URL cible HTTPS en conservant le chemin demandé.
        ...
        .listen(3001, function () {
          console.log(
            "Redirection HTTP (port 3001) -> HTTPS (port " + PORT + ")",
          );
        });
    })
    .catch(function (err) {
  ...
  // Mode développement simple: HTTP uniquement.
  initialiserBase()
    .then(function () {
